...
def validate_design_data() -> list[str]:
    ...
    if transition.get("current_safe_hub") not in {"violet_gym", "route_32", "union_cave", "azalea_town", "slowpoke_well"}:
        errors.append("current transition state must record Violet Gym as current safe hub")
    if transition.get("current_route") not in {"route_32", "union_cave", "azalea_town", "slowpoke_well"}:
        errors.append("current transition state must point current route to route_32 or union_cave")
    if transition.get("next_required_story_node") not in {"route_32_union_cave_road", "union_cave_first_entry", "azalea_first_arrival", "slowpoke_well_first_entry", "bugsy_gym_challenge"}:
