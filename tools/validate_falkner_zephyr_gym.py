...
def validate_design_data() -> list[str]:
    ...
    if transition.get("current_safe_hub") not in {"violet_gym", "route_32"}:
    ...
    if transition.get("current_route") != "route_32":
        errors.append("current transition state must point current route to route_32")
    if transition.get("next_required_story_node") not in {"route_32_union_cave_road", "union_cave_first_entry"}:
