...
def validate_design_data() -> list[str]:
    ...
    if transition.get("current_route") not in {"route_29", "route_30", "route_32"}:
        errors.append("current transition state must record Route 29 or a later Johto route")
    if transition.get("next_required_story_node") not in {"cherrygrove_first_arrival", "route_30_first_steps", "mr_pokemon_house_first_visit", "violet_city_first_arrival", "sprout_tower_first_floor", "sprout_tower_upper_floor", "falkner_gym_battle", "route_32_union_cave_road", "union_cave_first_entry"}:
