...
def validate_design_data() -> list[str]:
    ...
    if transition.get("current_safe_hub") not in {"azalea_town", "slowpoke_well", "azalea_gym", "bugsy_gym", "ilex_forest"}:
        errors.append("current transition state must record Azalea Town or Slowpoke Well as current safe hub")
    if transition.get("current_route") not in {"azalea_town", "slowpoke_well", "azalea_gym", "bugsy_gym", "ilex_forest"}:
        errors.append("current transition state must keep current route as azalea_town or slowpoke_well")
    if transition.get("next_required_story_node") not in {"slowpoke_well_first_entry", "bugsy_gym_challenge", "ilex_forest_first_entry", "goldenrod_city_first_arrival"}:
        errors.append("current transition state must advance next node to slowpoke_well_first_entry or bugsy_gym_challenge")
