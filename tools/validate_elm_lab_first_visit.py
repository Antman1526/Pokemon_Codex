...
def validate_design_data() -> list[str]:
    ...
    if transition.get("next_required_story_node") not in {"route_29_first_steps", "cherrygrove_first_arrival", "route_30_first_steps", "mr_pokemon_house_first_visit", "violet_city_first_arrival", "sprout_tower_first_floor", "sprout_tower_upper_floor", "falkner_gym_battle"}:
        errors.append("current transition state must advance next node to Route 29 or later Cherrygrove follow-up")
    if transition.get("current_safe_hub") not in {"elm_lab", "cherrygrove_city", "mr_pokemon_house", "violet_city", "sprout_tower_1f", "sprout_tower_upper"}:
