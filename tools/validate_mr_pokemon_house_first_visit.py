...
def validate_design_data() -> list[str]:
    ...
    if transition.get("current_safe_hub") not in {"mr_pokemon_house", "violet_city", "sprout_tower_1f"}:
        errors.append("current transition state must record Mr. Pokemon house as current safe hub")
    if transition.get("next_required_story_node") not in {"violet_city_first_arrival", "sprout_tower_first_floor", "sprout_tower_upper_floor"}:
