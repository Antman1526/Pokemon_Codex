...
def validate_design_data() -> list[str]:
    ...
    if transition.get("current_route") != "route_30":
        errors.append("current transition state must record current_route route_30")
    if transition.get("next_required_story_node") not in {"mr_pokemon_house_first_visit", "violet_city_first_arrival", "sprout_tower_first_floor", "sprout_tower_upper_floor", "falkner_gym_battle"}:
