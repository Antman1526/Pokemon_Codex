...
def validate_design_data() -> list[str]:
    ...
    if transition.get("current_route") not in {"route_30", "route_32", "union_cave", "azalea_town"}:
        errors.append("current transition state must record current_route route_30")
    if transition.get("next_required_story_node") not in {"mr_pokemon_house_first_visit", "violet_city_first_arrival", "sprout_tower_first_floor", "sprout_tower_upper_floor", "falkner_gym_battle", "route_32_union_cave_road", "union_cave_first_entry", "azalea_first_arrival", "slowpoke_well_first_entry"}:
