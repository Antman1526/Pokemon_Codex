...
def validate_design_data() -> list[str]:
    ...
    if transition.get("current_safe_hub") not in {"violet_city", "sprout_tower_1f", "sprout_tower_upper", "violet_gym", "route_32", "union_cave", "azalea_town", "slowpoke_well", "azalea_gym", "bugsy_gym", "ilex_forest"}:
        errors.append("current transition state must record Violet City as current safe hub")
    if transition.get("next_required_story_node") not in {"sprout_tower_first_floor", "sprout_tower_upper_floor", "falkner_gym_battle", "route_32_union_cave_road", "union_cave_first_entry", "azalea_first_arrival", "slowpoke_well_first_entry", "bugsy_gym_challenge", "ilex_forest_first_entry", "goldenrod_city_first_arrival"}:
