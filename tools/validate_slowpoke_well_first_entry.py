...
def validate_design_data() -> list[str]:
    ...
    if transition.get("current_safe_hub") not in {"slowpoke_well", "azalea_gym", "ilex_forest"}:
        errors.append("current transition state must record Slowpoke Well or Azalea Gym as current safe hub")
    if transition.get("current_route") not in {"slowpoke_well", "bugsy_gym", "ilex_forest"}:
        errors.append("current transition state must keep current route as slowpoke_well or bugsy_gym")
    if transition.get("next_required_story_node") not in {"bugsy_gym_challenge", "ilex_forest_first_entry", "goldenrod_city_first_arrival"}:
