...
def validate_design_data() -> list[str]:
    ...
    if transition.get("current_safe_hub") not in {"route_32", "union_cave", "azalea_town", "slowpoke_well"}:
    ...
    if transition.get("next_required_story_node") not in {"union_cave_first_entry", "azalea_first_arrival", "slowpoke_well_first_entry", "bugsy_gym_challenge"}:
