...
def validate_design_data() -> list[str]:
    ...
    if transition.get("current_safe_hub") not in {"union_cave", "azalea_town", "slowpoke_well"}:
        errors.append("current transition state must record Union Cave or Azalea Town as current safe hub")
    if transition.get("current_route") not in {"union_cave", "azalea_town", "slowpoke_well"}:
        errors.append("current transition state must keep current route as union_cave or azalea_town")
    if transition.get("next_required_story_node") not in {"azalea_first_arrival", "slowpoke_well_first_entry", "bugsy_gym_challenge"}:
