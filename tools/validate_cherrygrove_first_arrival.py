...
def validate_design_data() -> list[str]:
    ...
    if transition.get("current_safe_hub") not in {"cherrygrove_city", "mr_pokemon_house", "violet_city", "sprout_tower_1f", "sprout_tower_upper", "violet_gym", "route_32", "union_cave"}:
        errors.append("current transition state must record Cherrygrove as current safe hub")
    if transition.get("next_required_story_node") not in {"route_30_first_steps", "mr_pokemon_house_first_visit", "violet_city_first_arrival", "sprout_tower_first_floor", "sprout_tower_upper_floor", "falkner_gym_battle", "route_32_union_cave_road", "union_cave_first_entry", "azalea_first_arrival"}:
